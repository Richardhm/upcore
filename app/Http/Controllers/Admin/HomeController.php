...
class HomeController extends Controller
{
    public function index(Request $request)
    {
        if(auth()->user()->admin) {
           $user = User::find(auth()->user()->id);
            ...
            $totalComissao = DB::table('comissoes_corretor_lancados')->selectRaw('sum(valor) as total')->whereRaw("comissao_id IN (SELECT id FROM comissoes WHERE user_id = 2) AND MONTH(DATA) = MONTH(NOW()) AND status = 1")->first()->total;    
    ...
    {
        if($request->ajax()) {
            $comissoes = DB::table('comissoes_corretor_lancados')
